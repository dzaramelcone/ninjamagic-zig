...
pub const Registry = struct {
    ...
    pub fn upsert(self: *Registry, id: u16, name: []const u8) !void {
        try self.map.put(name, id);
    }

    pub fn idOf(self: *Registry, name: []const u8) ?u16 {
        return self.map.get(name);
    }
    pub fn matchPrefix(self: *Registry, prefix_raw: []const u8) ?u16 {
...
test "Registry – fuzz round-trip" {
    ...
    for (0..trials) |_| {
        ...
        for (orig, 0..) |c, i| {
            tmp_buf[i] = if (rng.boolean()) std.ascii.toUpper(c) else c;
        }
        const cased = tmp_buf[0..orig.len];
        try std.testing.expectEqual(reg.idOf(orig) orelse unreachable, reg.idOf(cased) orelse null);
        ...
        if (got) |gid| {
            ...
            try std.testing.expect(Registry.iStartsWith(nm, pref));
        } else {
            // there truly must be no matches
            var any = false;
            for (names.items) |nm| {
                if (Registry.iStartsWith(nm, pref)) any = true;
            }
